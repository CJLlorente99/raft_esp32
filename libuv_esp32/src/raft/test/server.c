                      ...
                      unsigned id)
{
    ...
    if (rv != 0) {
        ESP_LOGE("SERVER_INIT", "uv_timer_init(): %d", rv);
        goto err;
    }
    s->timer.data = s;
    ...
    if (rv != 0) {
        ESP_LOGE("SERVER_INIT", "raft_uv_init(): %s", s->io.errmsg);
    ...
    rv = FsmInit(&s->fsm);
    if (rv != 0) {
        ESP_LOGE("SERVER_INIT", "FsmInit(): %s", raft_strerror(rv));
    ...
    if (rv != 0) {
        ESP_LOGE("SERVER_INIT", "raft_init(): %s", raft_errmsg(&s->raft));
    ...
    for (i = 0; i < N_SERVERS; i++) {
        ...
        rv = raft_configuration_add(&configuration, server_id, address,
                                    RAFT_VOTER);
        if (rv != 0) {
            ESP_LOGE("SERVER_INIT", "raft_configuration_add(): %s", raft_strerror(rv));
...
static void serverApplyCb(struct raft_apply *req, int status, void *result)
{
    ...
    if (status != 0) {
        if (status != RAFT_LEADERSHIPLOST) {
            ESP_LOGE("SERVER_INIT", "raft_apply() callback: %s (%d)", raft_errmsg(&s->raft),
                 status);
        }
        return;
    }
...
static void serverTimerCb(uv_timer_t *timer)
{
    ...
    // TODO
    // new entry to the FSM inside buf.base and buf.len
    // the buffer should be a TreeFSM

    if (buf.base == NULL) {
        ESP_LOGE("serverTimerCb", "serverTimerCb(): out of memory");
        return;
    }
    ...
    if (rv != 0) {
        ESP_LOGE("serverTimerCb", "raft_apply(): %s", raft_errmsg(&s->raft));
        return;
    }
}
...
int main_raft(void* ignore)
{
    ...
    if (rv != 0) {
        ESP_LOGE("MAIN", "uv_loop_init(): %d", rv);
        goto err;
    }
    ...
    if (rv != 0) {
        ESP_LOGE("MAIN", "uv_signal_init(): %d", rv);
        goto err_after_server_init;
    }
    sigint.data = &server;
    ...
    if (rv != 0) {
        ESP_LOGE("MAIN", "uv_run_start(): %d", rv);
    }
